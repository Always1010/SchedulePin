import type { Wallpaper } from "./backgroundModel";
...
export async function exportWallpapers(items: Wallpaper[]) {
  const wallpapers = await Promise.all(items.filter(w => w.favorite).map(async ({ blob, thumbnail: _, ...meta }) => ({ ...meta, image: await dataUrl(blob) })));
...
export async function importWallpapers(file: File): Promise<Wallpaper[]> {
  ...
  for (const item of data.wallpapers) {
    ...
    items.push(await prepareWallpaper(blob, { id: crypto.randomUUID(), title: String(item.title ?? "导入壁纸").slice(0,200), author: String(item.author ?? "").slice(0,1000), source: item.source === "commons" ? "commons" : "local", sourceUrl: url(item.sourceUrl,"commons.wikimedia.org"), license: String(item.license ?? "").slice(0,200), licenseUrl: url(item.licenseUrl,"creativecommons.org"), favorite: true }));
  }
  return items;
}
