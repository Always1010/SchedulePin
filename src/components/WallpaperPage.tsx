...
export function WallpaperPage({ preferences, wallpapers, settings, tasks, navigation, navigationPreferences, onBack }: {
  ...
  const preparePhoto = async () => {
    const cached = selected ?? [...wallpapers].sort((a,b)=>b.createdAt-a.createdAt)[0];
    if (cached) return cached;
    try {
      const [item] = await discoverWallpapers(draft.category);
      return await downloadWallpaper(item);
    } catch {
      throw new Error("壁纸未能加载，当前背景未更改。请重试，或上传本地图片后保存。");
    }
  };
  const selectPhoto = () => {
    patch({style:"photo"});
    void run(async()=>{ choose(await preparePhoto()); });
  };
  ...
  return <main className="wallpaper-page">
    <div className="wallpaper-page-topbar">
      <header className="wallpaper-page-heading"><button type="button" disabled={busy} onClick={onBack}><ArrowLeft size={16}/>取消</button><div><h1>背景与壁纸</h1><p>当前为预览，点击“保存背景”后应用。</p></div><button type="button" className="wallpaper-save" disabled={busy} onClick={save}>{busy?"处理中…":"保存背景"}</button></header>
      {busy && <p className="wallpaper-feedback" role="status">{draft.style==="photo"&&!selected?"正在准备壁纸，首次加载可能需要一些时间…":"正在处理，请稍候…"}</p>}
    ...
    <div className="wallpaper-workspace"><section className="wallpaper-controls"><div className="wallpaper-tabs" role="tablist" aria-label="背景设置"><button role="tab" aria-selected={tab==="style"} onClick={()=>setTab("style")}>风格</button><button role="tab" aria-selected={tab==="library"} onClick={()=>setTab("library")}>我的壁纸</button><button role="tab" aria-selected={tab==="online"} onClick={()=>setTab("online")}>在线精选</button></div>
      ...
      {tab==="style" && <>
        ...
        {draft.style==="photo"&&!selected&&<div className="wallpaper-photo-empty"><p className="wallpaper-help">风景沉浸需要一张壁纸。可以自动加载，也可以使用自己的图片。</p><div className="wallpaper-library-actions"><button type="button" onClick={selectPhoto}>重新加载壁纸</button><button type="button" onClick={()=>setTab("library")}>使用本地图片</button></div></div>}
