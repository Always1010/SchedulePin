...
export function BackgroundToolbar({ preferences, wallpapers, current, onOpen, onWindowCurrent }: { preferences: BackgroundPreferences; wallpapers: Wallpaper[]; current?: Wallpaper; onOpen:()=>void; onWindowCurrent:(item?:Wallpaper)=>void }) {
  ...
  const committed=(saved:boolean)=>{if(!saved)throw new Error("设置已在其他窗口更新，请重试。");};
  const toggleFixed=async()=>{if(preferences.mode==="fixed")committed(await resumeWallpaperRotation(preferences.revision));else if(current)committed(await pinWallpaper(current,preferences.revision));onWindowCurrent(undefined);};
  return <>
    ...
    {candidate&&<Candidate item={candidate} busy={busy} onClose={()=>setCandidate(null)} onApply={()=>void run(async()=>{committed(await pinWallpaper(candidate,preferences.revision));onWindowCurrent(undefined);setCandidate(null);await trimBackgroundCache();})}/>}
  </>;
}
