...
export function OverallStyleEditor({ settings, onPatch, saving }: CommonProps) {
  ...
  return <section className="settings-card settings-detail-card appearance-controls">
    ...
    <label className="settings-control-label"><Type size={14} />默认字体</label><div className="font-options">{fonts.map(font => <button type="button" key={font.value} data-font={font.value} className={settings.fontFamily === font.value ? "selected" : ""} onClick={() => onPatch({ fontFamily: font.value })}><strong>{font.sample}</strong><small>{font.label}</small></button>)}</div>
    ...
    <div className="range-heading density-range-heading"><span><AlignJustify size={14} />布局间距</span><strong>{Math.round(settings.densityLevel)}%</strong></div><input className="range density-range" type="range" min="0" max="100" step="5" value={settings.densityLevel} onChange={event => { const densityLevel = Number(event.target.value); onPatch({ densityLevel, density: densityLevel < 34 ? "compact" : densityLevel > 66 ? "spacious" : "comfortable" }); }} /><div className="range-scale"><span>更紧凑</span><span>更宽松</span></div>
...
export function PrincipleStyleEditor({ settings, onPatch, saving, followBackground, onFollowChange }: CommonProps & { followBackground: boolean; onFollowChange: (follow: boolean) => void }) {
  ...
  return <section className="settings-card settings-detail-card appearance-controls">
    ...
    <>
      <label className="settings-control-label"><Type size={14} />卡片字体</label><div className="font-options">{fonts.map(font => <button type="button" key={font.value} data-font={font.value} className={settings.principleFontFamily === font.value ? "selected" : ""} onClick={() => onPatch({ principleFontFamily: font.value })}><strong>{font.sample}</strong><small>{font.label}</small></button>)}</div>
...
function Range({ label, value, min, max, step, current, onChange }: { label: string; value: string; min: string; max: string; step: string; current: number; onChange: (value: number) => void }) {
  return <><div className="range-heading"><span>{label}</span><strong>{value}</strong></div><input className="range" type="range" min={min} max={max} step={step} value={current} onChange={event => onChange(Number(event.target.value))} /></>;
}
