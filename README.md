...
- **背景与壁纸**：右下角“壁纸库”或设置中的“背景与壁纸”提供清爽纸感、流彩渐变和风景沉浸三种风格，可独立设置明暗、强调色、从图取色、遮罩、模糊、面板透明度和 Principle 跟随配色。横竖屏分别保存裁切位置，调整先预览，保存后应用。
- **壁纸收藏**：上传 JPG / PNG / WebP（20 MB 以内，优化到最长边 3840px），或从 Wikimedia Commons 的自然、城市、抽象分类选择图片。保留作者、来源和许可信息；收藏保存实际图片并支持离线使用，可导出 / 导入 JSON 图片备份。收藏操作不会自动固定背景。
- **自动换图**：支持固定、每日、每次打开或刷新三种模式；自动轮换可选在线图库或仅收藏。每日模式在当天首次打开时更新；“每次打开或刷新”由各窗口独立选图，打开、刷新或手动换图都不会带动已有窗口切换。正在输入、打开弹窗或页面隐藏时延后换图，网络失败保留上一张。固定后点击“换一张”先预览，确认才替换。新标签页背景不会修改 Windows 壁纸或触发桌面同步。
...
| `npm run test:wallpaper` | 检查上传、收藏备份、在线图库、草稿取消、裁切和离线图库 | 否 |
