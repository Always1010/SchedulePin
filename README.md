...
- **个性化设置**：设置首页按快捷访问、外观、Principle 和桌面展示分类；外观编辑器可调整主题、字体、字号、间距和圆角，并用共享页面组件预览横屏、竖屏或侧边栏。Principle 保留独立背景主题与文字样式。外观草稿仅在保存后生效。
...
前端回归测试脚本会先构建前端，再启动仅监听本机的临时产物预览服务，创建隔离浏览器上下文，并在结束后关闭服务和浏览器；测试数据不会写入日常浏览器配置。测试覆盖导航管理、完整任务列表、横竖屏、独立滚动、任务操作、外观草稿及主题，截图位于 `.tools/ui-tests/`。
