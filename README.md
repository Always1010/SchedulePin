...
- **新标签页总览**：左侧用紧凑的文字列表快速访问网站，右侧保留主题化的 `Principle` 和完整 `To-Do List`。大横屏和大竖屏都按可用宽度保持双栏，两栏分别滚动；`To-Do List` 收起后以横向摘要显示当天完成进度，并可在卡片内展开，Principle 也可独立收起，状态会被记住。
- **快捷访问**：只填网址即可添加，名称选填；留空时自动展示网站简称，例如 `https://docs.example.com` 显示为 `example`，并随网址更新，已有自定义名称保留。支持固定、分组、查找、编辑、删除和手动排序。首次使用从空列表开始；链接默认在当前标签页打开，Ctrl / 中键沿用浏览器的新标签打开方式。设置中可切换导航左右位置和网站简称显示；名称与简称相同时不会重复展示。
