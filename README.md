...
- **个性化设置**：导航分为“浏览器页面”和“桌面展示”。浏览器页面按整体样式、页面背景、快捷导航、待办区域和原则卡片组织；桌面展示在同一页连接助手、选择显示范围和显示器，并直接编辑当前屏幕的计划布局。整体样式统一管理默认主题、字体、字号、间距、圆角，以及新标签页单独配色和面板透明度；Principle 的内容、独立样式和跟随配色集中在原则卡片。修改即时生效并自动保存，失败可重试，各页恢复默认只重置相应范围。
