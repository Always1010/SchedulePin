...
- **个性化设置**：设置首页按外观、Principle 和桌面展示分类；外观编辑器可一边调整主题、字体、字号、界面密度和卡片圆角，一边预览完整页面或侧边栏。调整内容在保存前只作为草稿，不会影响正在使用的界面。
- **多入口访问**：点击扩展图标可在浏览网页时打开侧边栏计划；侧边栏顶部可随时打开完整页面。插件不会强制替换浏览器的新标签页，是否把完整页面设为启动页由用户自行选择。
...
5. 点击工具栏中的 SchedulePin 图标打开侧边栏，再点击顶部“完整页面”即可在新标签页中打开完整计划。

如希望启动浏览器时自动显示 SchedulePin，可先通过侧边栏打开完整页面，复制地址栏中的扩展页面地址，再在 Chrome 的“设置 → 启动时”或 Edge 的“设置 → 开始、主页和新建标签页”中将该地址添加为启动页。此设置完全可选，插件安装后不会主动接管新标签页。

`npm run dev` 和 `npm run preview` 只适合快速查看 React 界面。普通网页没有扩展权限，因此不能用它们验证侧边栏、从侧边栏打开完整页面或桌面助手连接。
...
只开发任务管理、完整页面、侧边栏或设置页面时，不需要 Rust。通过命令总览中的 `dev` 命令快速调整界面，再用 `extension:build` 生成真正的插件资源并按“加载浏览器插件”步骤验证扩展能力。
