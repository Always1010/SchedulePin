...
- 新标签页：顶部突出展示“纪律”，下方用一张连续的 To-Do List 管理所有待办，不再拆分重点、日程和备忘。
- 快速添加与排序：在列表末尾输入内容并按 Enter 即可添加，拖动任务左侧手柄可调整顺序。
- 任务归档：当天完成的任务保留删除线，次日自动移出列表；也可立即归档，并在归档页恢复或删除。
...
它现在就在本机项目目录中，不需要服务器。以后正式发布时，可把这个文件和插件包放在 GitHub Releases；当前版本没有伪造一个不可用的网络下载按钮。
...
- 普通 To-Do 不受“当天”限制，未完成时会持续保留；纪律按天重置完成状态。
- 浏览器预览模式使用 `localStorage`，加载为扩展后使用 `chrome.storage.local`，两者不是同一份数据。
- 旧 Tauri / SQLite 原型数据不会自动迁移到 0.2 插件；旧版仅为测试原型，迁移层已随旧架构一同移除。
